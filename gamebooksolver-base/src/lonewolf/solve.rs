...
use crate::lonewolf::mini::{CharacterConstant, CharacterVariable, Equipment, Flag, NextStep};
...
pub fn step<P: Rational>(
    ...
    match ns {
        NextStep::NewChapter(cid, cvar) => {
            ...
            let mut out: Vec<Choice<P, String, NextStep>> = flatten_decision(ccst, cvar, &chapter.pchoice)
                .into_iter()
            ...
            let scorer = |outcome: &Outcome<P, NextStep>| outcome.iter().map(|r| scorei(&r.v)).max().unwrap_or(0);
...
fn get_score<P: Rational, F>(scorer: F, tgt: &[ChapterId], ns: &NextStep) -> Option<P>
where
...
pub fn solve_lws<P: Rational + Eq + std::hash::Hash, F>(
    scorer: F,
    tgt: &[ChapterId],
    ...
    F: Fn(Equipment, Flags) -> P,
{
    ...
    let o = solve::<NextStep, _, _, _, _, _>(
...
mod test {
    ...
    use crate::lonewolf::mini::{Book, Item};
    ...
    fn dummy_book() {
        ...
        let sol = solve_lws::<BigRational, _>(
