...
use crate::lonewolf::mini::{
  Book, CharacterConstant, CharacterVariable, Equipment, Item, NextStep,
};
  ...
  match outcome {
    ...
    ChapterOutcome::Fight(fd, nxt) => {
      ...
      let out: Outcome<P, NextStep> = cinfo
        ...
        .into_iter()
        .flat_map(|r| {
          let (noutcome, charendurance): (ChapterOutcome<P>, Endurance) = match r.v {
            Escaped::Std(n) => {
              let x: &ChapterOutcome<P> = nxt;
              (x.clone(), n)
            }
...
mod test {
  ...
  fn oneround() {
    ...
    let duel = ChapterOutcome::OneRound::<BigRational>(
      FightDetails {
    ...
    let mut expected = vec![
      Proba {
        v: Some(1),
        p: BigRational::from_i64(3, 10),
      },
      Proba {
        v: Some(2),
        p: BigRational::from_i64(1, 10),
      },
      Proba {
        v: Some(3),
        p: BigRational::from_i64(6, 10),
      },
    ];
    expected.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
    assert_eq!(actual, expected);
