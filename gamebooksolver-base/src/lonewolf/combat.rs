...
use crate::lonewolf::mini::{CharacterConstant, CharacterVariableG, Discipline, Equipment, Flag, Flags, Item, Weapon};
use crate::solver::base::{Cache, Outcome, Proba, optimize_outcome};
...
impl<P: Rational + Ord + std::hash::Hash + std::iter::Sum> CombatInfo<P> {
    ...
    pub fn get_ratio(&self) -> CombatSkill {
        ...
        if modifiers.contains(&FightModifier::BareHanded) || self.weapons.is_empty() {
            ...
            && (self.weapons.contains(&wspec) || (wspec == Weapon::Spear && self.weapons.contains(&Weapon::MagicSpear)))
        {
            o += 2
        }
