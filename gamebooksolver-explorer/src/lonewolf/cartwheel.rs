...
fn sol_for_target(target: usize) -> Vec<Outcome<u8>> {
    ...
    (0..target + 1)
        .map(|start_money| {
            (0..target + 7)
                .map(|end_money| Proba {
                    p: Rational::from_f64(o.get(start_money, end_money)).unwrap(),
                    v: end_money as u8,
                })
                .filter(|pb| pb.p != 0)
                .collect()
...
lazy_static! {
    static ref CARTWHEEL: Vec<Vec<Outcome<u8>>> = (20..50).map(sol_for_target).collect();
}

pub fn cartwheel(starting_money: u8, target: u8) -> Outcome<u8> {
    if starting_money >= target {
        vec![Proba::certain(target)]
    } else if !(20..50).contains(&target) {
        panic!("Target must be >= 20 && < 50, was {}", target)
    } else {
        CARTWHEEL[target as usize - 20][starting_money as usize].clone()
    }
}
