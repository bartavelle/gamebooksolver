...
use gamebooksolver_base::lonewolf::mini::{
    mkchar, Book, CVarState, CharacterConstant, CharacterVariable, CompactSolution, CompactState, Discipline,
    Equipment, Flag, Flags, Item, NextStep, SolDesc, SolutionDump, Weapon,
};
...
#[derive(Debug, StructOpt)]
#[structopt(name = "solutionexplorer", about = "Explore solutions generated by gamebooksolver.")]
struct Opt {
...
fn mksol(ini: NextStep, sttmap: HashMap<NextStep, ChoppedSolution<Rational, NextStep>>) -> HashMap<u16, ChapterStats> {
    ...
    fn go(
        ...
        let mut update_choice = |p: Rational, cstt: &NextStep, memo: &mut HashMap<u16, ChapterStats>| {
...
fn decode_buffer<R: std::io::Read>(r: &mut R) -> gamebooksolver_base::lonewolf::mini::SolutionDump<Rational> {
    let mut buf = Vec::new();
...
fn compare_sols(
    ...
    let mkmap = |m: HashMap<NextStep, ChoppedSolution<Rational, NextStep>>| -> HashMap<NextStep, Ocs> {
        m.into_iter()
            .map(|(ns, cs)| {
                let ch = ns.chapter();
                (ns, descored(ch, cs))
...
impl<I: Write> minicbor::encode::Write for WW<I> {
    ...
    fn write_all(&mut self, buf: &[u8]) -> std::result::Result<(), <Self as minicbor::encode::Write>::Error> {
    ...
    match &opt.cmd {
        ...
        Some(Subcommand::Soldump(cnt)) => {
            ...
            let mscoremap = cnt
                .resultspath
                .as_ref()
                .map(|pth| load_results(cnt.discipline.iter().copied().collect(), &iitems, &iflags, pth));
