...
flattenDecision :: (String -> a) -> CharacterConstant -> CharacterVariable -> Decision -> [([a], ChapterOutcome)]
flattenDecision cnv cconstant cvariable d =
  let inventory = cvariable ^. equipment
      withEffects effects nxt = do
        let nvariable = foldl' (updateSimple cconstant) cvariable effects
        (dsc, o) <- flattenDecision cnv cconstant nvariable nxt
        return (cnv (show effects) : dsc, Simple effects o)
   in case d of
        _ -- can be healed, finally!
          | hasFlag Poisonned2 cvariable && hasItem Laumspur inventory ->
            let nvariable = updateSimple cconstant cvariable (HealPlayer 4) & equipment %~ delItem Laumspur 1 & flag Poisonned2 .~ False
             in flattenDecision cnv cconstant nvariable d
        NoDecision o
          ...
          | hasCombat o && hasItem StrengthPotion4 inventory && hasItem StrengthPotion inventory ->
            [ ([cnv "don't use strength potion"], o),
              ([cnv "use strength potion"], Simple [LoseItem StrengthPotion 1, SetFlag StrengthPotionActive] o),
              ([cnv "use potent strength potion"], Simple [LoseItem StrengthPotion4 1, SetFlag PotentStrengthPotionActive] o)
            ]
        ...
        RetrieveEquipment nxt ->
          ...
          let CharacterVariable e flgs cure stoe = cvariable
              itms = items cure
           in flattenDecision cnv cconstant (CharacterVariable e flgs stoe 0) (foldr (uncurry CanTake) nxt itms)
        AfterCombat nxt
          ...
          | otherwise -> nopotion
          where
            nopotion = flattenDecision cnv cconstant cvariable nxt
        ...
        Conditional bc d' ->
          if check cconstant cvariable bc
            then flattenDecision cnv cconstant cvariable d'
            else []
        Special Portholes -> [([], Goto 197)]
        Special Cartwheel -> map (first (map cnv)) (cartwheel (cvariable ^. equipment . gold))
        EvadeFight nrounds cid fdetails co ->
          [ ([cnv "no evasion"], Fight fdetails co),
            ([cnv "evasion"], Fight (fdetails & fightMod %~ ((if nrounds > 0 then Timed nrounds (Evaded cid) else Evaded cid) :)) co)
          ]
        Decisions lst -> do
          (cdesc, d') <- lst
          (alldesc, o) <- flattenDecision cnv cconstant cvariable d'
          return (cnv cdesc : alldesc, o)
        CanTake item count nxt
          | item == Gold -> withEffects [GainItem Gold count] nxt
          | count <= 0 -> error "invalid count :("
          | count == 1 ->
            case canTake item cconstant cvariable of
              Nope -> notake
              SpaceAvailable n | n <= 0 -> error "space available invariant wrong"
              SpaceAvailable _ -> withEffects [GainItem item 1] nxt
              Mustdrop drops
                | not (importantItem item cconstant cvariable) -> notake
                | otherwise -> do
                  itemDropped <- drops
                  withEffects [LoseItem itemDropped 1, GainItem item 1] nxt
          | otherwise -> flattenDecision cnv cconstant cvariable (CanTake item 1 (CanTake item (count - 1) nxt))
          where
            notake = flattenDecision cnv cconstant cvariable nxt
        Canbuy item price nxt
          ...
          | not (importantItem item cconstant cvariable) ->
            nobuy
          | otherwise ->
            nobuy ++ case canTake item cconstant cvariable of
              Nope -> nobuy
          ...
          where
            nobuy = flattenDecision cnv cconstant cvariable nxt
        Cansell item price nxt
          | not (hasItem item inventory) -> nosell
          | otherwise -> nosell ++ sell
          where
            nosell = flattenDecision cnv cconstant cvariable nxt
            sell = withEffects [GainItem Gold price, LoseItem item 1] d
        Special B05S127 ->
          let cs = fromIntegral (cconstant ^. combatSkill)
           ...
           in if b1 + b2 /= 1
                ...
                else [([cnv "No decision"], Randomly [(b1, Goto 159), (b2, Goto 93)])]
        RemoveItemFrom BackpackSlot n nxt
          | n <= 0 -> flattenDecision cnv cconstant cvariable nxt
          | otherwise ->
        ...
        Special B05S357 ->
          let lwe = cvariable ^. curendurance
           ...
           in [([cnv "Search the sentry body"], result 207), ([cnv "Ignore the sentry body"], result 224)]
