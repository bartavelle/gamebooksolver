module LoneWolf.Simplify (simplify, path, getLinks, forSimplification, extractMultiFight) where
...
extractMultiFight chaps = std ++ mkMulti (maxid + 1) multi
  where
      ...
      case separateOutcome c of
        ...
        Just (mkdec, co) ->
          case extractSubfights co of
            ([], _) -> Right ch
            (fights, fin) -> Left (CI cid nm dsc mkdec fights fin)
    ...
    mkSimplified cid (CI rcid n d mkdec details oc) = (lastsol + 2, (rcid, Chapter n d (mkdec (Goto cid))) : sols ++ [dummyChapter (lastsol + 1) oc])
      where
