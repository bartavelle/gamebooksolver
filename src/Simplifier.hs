...
uselessEdges ::
  ...
  (Ord chapterid, Ord priority, Show chapterid) =>
  (chapterid -> priority) ->
...
prunePathes childs forbidden start end
  ...
  where
    ...
    onlyOneNeutral lst =
      let (neutral, others) = partition ((== Neutral) . snd) lst
...
graphMap ::
  forall chapterid priority.
  (Show chapterid, Ord chapterid, Ord priority, Show priority) =>
  (chapterid -> priority) ->
  (chapterid -> M.Map (S.Set chapterid) LinkType) ->
  (chapterid -> S.Set chapterid) ->
  chapterid ->
